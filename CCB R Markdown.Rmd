---
title: 'Reliability and validity of the compulsory citizenship behaviors scale: 6-Step
  R-based psychometrics protocol among nurses in Turkey'
author: "Bora YILDIZ"
date: "2/9/2021"
output:
...
for (n in c('readxl','haven','GPArotation', 'corrplot', 'ggpubr', 'SparseM', 'foreign', 'utils', 'relimp', 
            ...
            'semTools','semPlot', 'qgraph','sem','CTT',
            'MBESS','cluster')) {if(!require(n,character.only=TRUE)){install.packages(n)}}
library(n,character.only=TRUE)

# Reading the data
mydata <- read_sav("CCB_530(noout).sav")
names(mydata)
dim(mydata)
...
mydata$CCB_F = apply(mydata[ , factor1], 1, mean) ##creates average scores
attach(mydata)
names(mydata)
mean(CCB_F)
sd(CCB_F)
